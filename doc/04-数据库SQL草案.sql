...
CREATE INDEX IF NOT EXISTS idx_message_role_type
    ON message (role, message_type);
...
CREATE TRIGGER IF NOT EXISTS trg_message_ai
...
BEGIN
    INSERT INTO message_fts (
        ...
        COALESCE(NEW.raw_chunk, '')
    );
END;

CREATE TRIGGER IF NOT EXISTS trg_message_au
...
CREATE TRIGGER IF NOT EXISTS trg_message_ad
...
INSERT OR IGNORE INTO config (
    ...
    ('cfg_runtime_default_project_path', 'runtime', 'defaultProjectPath', 'string', 'D:\\Project\\ali\\260409'),
    ('cfg_runtime_default_shell', 'runtime', 'defaultShell', 'string', 'powershell'),
    ('cfg_storage_db_path', 'storage', 'dbPath', 'string', '%LOCALAPPDATA%\\mutiAgent\\data\\muti-agent.db'),
    ('cfg_storage_session_log_retention_days', 'storage', 'sessionLogRetentionDays', 'number', '30'),
    ('cfg_ui_default_terminal_mode', 'ui', 'defaultTerminalMode', 'string', 'raw'),
    ('cfg_session_auto_reconnect', 'session', 'autoReconnect', 'boolean', 'false');
